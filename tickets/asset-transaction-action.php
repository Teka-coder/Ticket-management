...
include "../phpqrcode/qrlib.php";
    if (isset($_POST['asset_transfer_form']) and $_POST['tcustodian']!= $_POST['fcustodian'] or $_POST['tcompany']!= $_POST['fcompany'] or $_POST['tdepartment']!= $_POST['fdepartment'] or $_POST['twarehouse']!=$_POST['fwarehouse']){ 
 ...
 if(isset($_POST['selected_accessory'])) {
     ...
     $query2="UPDATE accessories SET custodian=? WHERE id=?;";
      ...
      $comm_query2->execute();
     	 $sel1="SELECT accessory_name FROM accessories WHERE id=?;";
    
      $cmd1=$conn->prepare($sel1) or die(mysqli_connect_error($conn));
      $cmd1->bind_param("s",$value1);
      $cmd1->execute();
      $res1=$cmd1->get_result();
      if ($res1->num_rows>0) 
        $row1=$res1->fetch_assoc();
     $received.=$row1["accessory_name"].",";
}
	 foreach ($_POST['unselectedAcc'] as $key2 => $value2) {
      ...
      $cmd2->bind_param("s",$value2);
      $cmd2->execute();
      $res2=$cmd2->get_result();
      if ($res2->num_rows>0) 
        $row2=$res2->fetch_assoc();
     $released.=$row2["accessory_name"].":_:";
}
  ...
  foreach ($_POST['all_accessory'] as $key3 => $value3) {
      ...
      $cmd3->bind_param("s",$value3);
      $cmd3->execute();
      $res3=$cmd3->get_result();
      if ($res3->num_rows>0) 
        $row3=$res3->fetch_assoc();
   $received.=$row3["accessory_name"].":_:";
}
  }
 ...
 while($check_rec=$check_res->fetch_assoc()){
  if ($check_rec["Affected"]=='1') {
   ...
   $orig_sql="INSERT INTO item_transaction(item,from_warehouse,to_warehouse,from_custodian,to_custodian,from_dep,to_dep,from_comp,to_comp,new_added,received_acc,released_acc,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $notyet="Not yet";
        $exec=$conn->prepare($orig_sql) or die(mysqli_connect_error($conn));
        $exec->bind_param("sssssssssssss",$id ,$check_rec["warehouse"],$check_rec["warehouse"],$check_rec["custodian"],$check_rec["custodian"],$check_rec["department"],$check_rec["department"],$check_rec["company"],$check_rec["company"],$notyet,$notyet,$notyet,$by);
        $exec->execute();
  ...
  $command->bind_param("sssssssssssss",$id,$fwarehouse,$twarehouse,$fcustodian,$tcustodian,$fdep,$tdep,$fcompany,$tcompany,$newadded,$received,$released,$by);
  $command->execute();

 // echo $sql;
 if ($command) {
  $val=1;
           $update_sql="UPDATE items SET warehouse=?,custodian=?,department=?,company=?,Affected=? WHERE id=?";
           $update_command=$conn->prepare($update_sql) or die(mysqli_connect_error($conn));
           $update_command->bind_param("ssssss",$twarehouse,$tcustodian,$tdep,$tcompany,$val,$id);
           $update_command->execute();
           $_SESSION['completed']=$itemname.' trasfered successfully !';
           
}
 else {
   $_SESSION['completed']="Unable to transfer item";
